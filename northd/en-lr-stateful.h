...
#define LR_STATEFUL_TABLE_FOR_EACH_IN_P(LR_SFUL_REC, JOBID, TABLE) \
    HMAP_FOR_EACH_IN_PARALLEL (LR_SFUL_REC, key_node, JOBID, \
                               &(TABLE)->entries)
...
struct lr_stateful_tracked_data {
    ...
    struct hmapx deleted; /* Stores 'struct lr_stateful_record'. */
};
...
struct ed_type_lr_stateful {
    ...
    bool tracked;
    struct lr_stateful_tracked_data tracked_data;
};
