    ...
    struct lflow_ref *lflow_ref)
{
    ...
    ovs_assert(op->od == lr_stateful_rec->od);
    ...
    struct lflow_ref *lflow_ref)
{
    ...
    ovs_assert(op->peer->od == lr_stateful_rec->od);
    ...
    struct lflow_ref *lflow_ref)
{
    ...
    for (int i = 0; i < lr_stateful_rec->lrnat_rec->n_nat_entries; i++) {
        struct ovn_nat *nat_entry =
            &lr_stateful_rec->lrnat_rec->nat_entries[i];
...
build_lrouter_nat_defrag_and_lb(
    ...
    struct lflow_ref *lflow_ref)
{
    const struct ovn_datapath *od = lr_stateful_rec->od;
    ovs_assert(od->nbr);
...
build_routable_flows_for_router_port(
    ...
    struct ds *actions)
{
    ovs_assert(lrp->nbrp && lrp->od == lr_stateful_rec->od);
...
build_lrp_lflows_for_lbnats(struct ovn_port *op,
                            ...
                            struct lflow_table *lflows)
{
    ovs_assert(op->nbrp && op->od == lr_stateful_rec->od);
...
build_lr_stateful_flows(const struct lr_stateful_record *lr_stateful_rec,
                        ...
                        const struct chassis_features *features)
{
    build_lrouter_nat_defrag_and_lb(lr_stateful_rec, lflows, ls_ports,
                                    lr_ports, match, actions, meter_groups,
                                    features, lr_stateful_rec->lflow_ref);
    build_lr_gateway_redirect_flows_for_nats(lr_stateful_rec->od,
                                             lr_stateful_rec->lrnat_rec,
                                             lflows, match, actions,
                                             lr_stateful_rec->lflow_ref);
    build_lrouter_arp_nd_for_datapath(lr_stateful_rec->od,
                                      lr_stateful_rec->lrnat_rec, lflows,
                                      meter_groups,
                                      lr_stateful_rec->lflow_ref);
}
    ...
    const char *svc_monitor_mac)
{
    ...
    if (parallelization_state == STATE_USE_PARALLELIZATION) {
        ...
        LR_STATEFUL_TABLE_FOR_EACH (lr_stateful_rec, lr_stateful_table) {
            build_lr_stateful_flows(lr_stateful_rec, lsi.lflows,
                                ...
                                struct lflow_table *lflows)
{
    ...
    HMAPX_FOR_EACH (hmapx_node, &trk_data->crupdated) {
        ...
        build_lr_stateful_flows(lr_stateful_rec, lflows, lflow_input->ls_ports,
                                lflow_input->lr_ports, &match, &actions,
                                lflow_input->meter_groups,
                                lflow_input->features);

        /* Sync the new flows to SB. */
        lflow_ref_sync_lflows(lr_stateful_rec->lflow_ref, lflows, ovnsb_txn,
                              lflow_input->ls_datapaths,
                              lflow_input->lr_datapaths,
                              lflow_input->ovn_internal_version_changed,
                              lflow_input->sbrec_logical_flow_table,
        ...
        HMAP_FOR_EACH (op, dp_node, &lr_stateful_rec->od->ports) {
            ...
            lflow_ref_sync_lflows(op->stateful_lflow_ref, lflows,
                                  ovnsb_txn,
                                  lflow_input->ls_datapaths,
                                  lflow_input->lr_datapaths,
                                  lflow_input->ovn_internal_version_changed,
