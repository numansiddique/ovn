...
/* Adds a row with the specified contents to the Logical_Flow table. */
static void
ovn_lflow_add_objdep_ref(struct hmap *lflow_map, const struct ovn_datapath *od,
                         const unsigned long *dp_bitmap, size_t dp_bitmap_len,
                         enum ovn_stage stage, uint16_t priority,
...
#define ovn_lflow_add_with_lport_lflow_ref(LFLOW_MAP, OD, STAGE, PRIORITY, \
                                           MATCH, ACTIONS, IN_OUT_PORT, \
                                           CTRL_METER, STAGE_HINT, \
                                           LFLOW_DEP_MGR, LPORT_NAME) \
    ovn_lflow_add_objdep_ref(LFLOW_MAP, OD, NULL, 0, STAGE, PRIORITY, MATCH, \
                             ACTIONS, IN_OUT_PORT, CTRL_METER, STAGE_HINT, \
                             OVS_SOURCE_LOCATOR, LFLOW_DEP_MGR, \
                             OBJDEP_TYPE_LPORT, LPORT_NAME)
            ...
            for (size_t k = 0; k < rp->n_lsp_addrs; k++) {
                for (size_t l = 0; l < rp->lsp_addrs[k].n_ipv4_addrs; l++) {
                    ...
                    ovn_lflow_add_with_lport_lflow_ref(
                        ...
                        NULL, &op->nbsp->header_, dep_mgr, op->key);
                }
                                         ...
                                         struct objdep_mgr *lflow_dep_mgr)
{
    ...
    build_lswitch_port_sec_op(op, lflows, actions, match, lflow_dep_mgr);
    build_lswitch_learn_fdb_op(op, lflows, actions, match, lflow_dep_mgr);
    ...
    build_lswitch_dhcp_options_and_response(op, lflows, meter_groups,
                                            lflow_dep_mgr);
    build_lswitch_external_port(op, lflows, lflow_dep_mgr);
    build_lswitch_ip_unicast_lookup(op, lflows, actions, match, lflow_dep_mgr);
                                ...
                                struct objdep_mgr *lflow_dep_mgr)
{
    ...
    if (parallelization_state == STATE_USE_PARALLELIZATION) {
        ...
        for (index = 0; index < build_lflows_pool->size; index++) {
            ...
            lsiv[index].lflow_dep_mgr = lflow_dep_mgr;
...
lflow_data_init(struct lflow_data *data)
{
    ...
    objdep_mgr_init(&data->lflowobj_dep_mgr);
}
...
lflow_data_destroy(struct lflow_data *data)
{
    ...
    objdep_mgr_destroy(&data->lflowobj_dep_mgr);
}
...
static bool
delete_lflow_for_lsp(struct ovn_port *op OVS_UNUSED, bool is_update OVS_UNUSED,
                     const struct sbrec_logical_flow_table *sb_lflow_table  OVS_UNUSED,
                     struct hmap *lflows OVS_UNUSED)
{
#if 0
