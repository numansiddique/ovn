                                  ...
                                  struct lflow_ref *lflow_ref)
{
    ...
    const struct lr_lb_nat_data_record *lr_lbnat_rec;
    lr_lbnat_rec = lr_lb_nat_data_table_find_by_index(lr_lbnats, op->peer->od->index);
    ovs_assert(lr_lbnat_rec);
                                  ...
                                  struct lflow_ref *routable_lflow_ref)
{
    ...
    const struct lr_lb_nat_data_record *lr_lbnat_rec;
    lr_lbnat_rec = lr_lb_nat_data_table_find_by_index(lr_lbnats, op->od->index);
    ovs_assert(lr_lbnat_rec);
...
reset_lflow_refs_for_northd_resources(struct lflow_input *lflow_input)
{
    ...
    LS_LBACLS_TABLE_FOR_EACH (ls_lbacls_rec, lflow_input->ls_lbacls) {
        lflow_ref_reset(ls_lbacls_rec->lflow_ref);
    }
                                ...
                                struct lflow_table *lflows)
{
    ...
    HMAPX_FOR_EACH (hmapx_node, &trk_data->crupdated) {
        ...
        lflow_ref_clear_lflows(ls_lbacls_rec->lflow_ref);
