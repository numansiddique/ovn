...
#define ovn_lflow_add_with_hint(LFLOW_MAP, OD, STAGE, PRIORITY, MATCH, \
                                ACTIONS, STAGE_HINT, LFLOW_REF_LIST) \
    ovn_lflow_add_at(LFLOW_MAP, OD, NULL, 0, STAGE, PRIORITY, MATCH, ACTIONS, \
...
#define ovn_lflow_add_with_dp_group(LFLOW_MAP, DP_BITMAP, DP_BITMAP_LEN, \
    ...
    ovn_lflow_add_at(LFLOW_MAP, NULL, DP_BITMAP, DP_BITMAP_LEN, STAGE, \
                     ...
                     OVS_SOURCE_LOCATOR, LFLOW_REF_LIST)

#define ovn_lflow_add_default_drop(LFLOW_MAP, OD, STAGE, LFLOW_REF_LIST) \
...
#define ovn_lflow_add(LFLOW_MAP, OD, STAGE, PRIORITY, MATCH, ACTIONS, \
                      LFLOW_REF_LIST) \
    ovn_lflow_add_at(LFLOW_MAP, OD, NULL, 0, STAGE, PRIORITY, MATCH, ACTIONS, \
                     NULL, NULL, NULL, OVS_SOURCE_LOCATOR, LFLOW_REF_LIST)
...
#define ovn_lflow_metered(LFLOW_MAP, OD, STAGE, PRIORITY, MATCH, ACTIONS, \
    ...
    ovn_lflow_add_with_hint__(LFLOW_MAP, OD, STAGE, PRIORITY, MATCH, \
                              ACTIONS, NULL, CTRL_METER, NULL, LFLOW_REF_LIST)
                                ...
                                struct ds *actions, struct ds *match)
{
    ...
    if (!lsp_is_enabled(op->nbsp)) {
        ...
        ovn_lflow_add_with_lport_and_hint(
            ...
            op->key, &op->nbsp->header_, &op->lflows);

        ds_clear(match);
        ds_put_format(match, "outport == %s", op->json_key);
        ovn_lflow_add_with_lport_and_hint(
            lflows, op->od, S_SWITCH_IN_L2_UNKNOWN, 50, ds_cstr(match),
            debug_drop_action(), op->key, &op->nbsp->header_, &op->lflows);
        return;
    }
    ...
    if (lsp_is_vtep(op->nbsp)) {
        ...
        if (lsp_is_localnet(op->nbsp)) {
            ...
            ovn_lflow_add_with_lport_and_hint(lflows, op->od,
                    ...
                    &op->od->localnet_ports[0]->nbsp->header_, &op->lflows);
        }
    }
}
...
{
    ovs_assert(od->nbs);
    ovn_lflow_add(lflows, od, S_SWITCH_IN_LOOKUP_FDB, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_IN_PUT_FDB, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_IN_L2_LKUP, 0, "1",
                  "outport = get_fdb(eth.dst); next;", NULL);
}
                              ...
                              struct hmap *lflows)
{
    ...
    ovn_lflow_add(lflows, od, S_SWITCH_OUT_CHECK_PORT_SEC, 100,
                  "eth.mcast", REGBIT_PORT_SEC_DROP" = 0; next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_OUT_CHECK_PORT_SEC, 0, "1",
                  REGBIT_PORT_SEC_DROP" = check_out_port_sec(); next;", NULL);
               ...
               struct hmap *lflows)
{
     ...
     * allowed by default. */
    ovn_lflow_add(lflows, od, S_SWITCH_IN_PRE_ACL, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_OUT_PRE_ACL, 0, "1", "next;", NULL);

    ovn_lflow_add(lflows, od, S_SWITCH_IN_PRE_ACL, 110,
                  "eth.dst == $svc_monitor_mac", "next;", NULL);

    ovn_lflow_add(lflows, od, S_SWITCH_OUT_PRE_ACL, 110,
                  "eth.src == $svc_monitor_mac", "next;", NULL);
    ...
    if (od->has_stateful_acl) {
        ...
        ovn_lflow_add(lflows, od, S_SWITCH_IN_PRE_ACL, 100, "ip",
                      REGBIT_CONNTRACK_DEFRAG" = 1; next;", NULL);
             ...
             struct hmap *lflows)
{
    ...
    ovn_lflow_add(lflows, od, S_SWITCH_IN_PRE_LB, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_OUT_PRE_LB, 0, "1", "next;", NULL);
    ...
    ovn_lflow_add(lflows, od, S_SWITCH_IN_PRE_LB, 110,
                  REGBIT_ACL_STATELESS" == 1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_OUT_PRE_LB, 110,
                  REGBIT_ACL_STATELESS" == 1", "next;", NULL);
    ...
    if (od->has_lb_vip) {
        ovn_lflow_add(lflows, od, S_SWITCH_IN_PRE_LB,
                      100, "ip", REGBIT_CONNTRACK_NAT" = 1; next;", NULL);
                   ...
                   struct hmap *lflows)
{
     ...
     * allowed by default. */
    ovn_lflow_add(lflows, od, S_SWITCH_IN_PRE_STATEFUL, 0, "1", "next;", NULL);
    ...
    ovn_lflow_add(lflows, od, S_SWITCH_IN_PRE_STATEFUL, 110,
                  REGBIT_CONNTRACK_NAT" == 1", ct_lb_action, NULL);
    ...
    ovn_lflow_add(lflows, od, S_SWITCH_IN_PRE_STATEFUL, 100,
                  REGBIT_CONNTRACK_DEFRAG" == 1", "ct_next;", NULL);
                ...
                struct hmap *lflows)
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(stages); i++) {
        ...
        if (!od->has_acls && !od->has_lb_vip) {
            ovn_lflow_add(lflows, od, stage, UINT16_MAX, "1", "next;", NULL);
        } else {
            ovn_lflow_add(lflows, od, stage, 0, "1", "next;", NULL);
        }
                        ...
                        struct ds *actions)
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(stages); i++) {
        ...
        ovn_lflow_add(lflows, od, stage, 0, "1", ds_cstr(actions), NULL);
           ...
           const struct shash *meter_groups)
{
    ...
    if (!od->has_acls) {
        ...
        ovn_lflow_add(lflows, od, S_SWITCH_IN_ACL_EVAL, 0, "1", "next;", NULL);
        ovn_lflow_add(lflows, od, S_SWITCH_OUT_ACL_EVAL, 0, "1", "next;",
                      NULL);
        ovn_lflow_add(lflows, od, S_SWITCH_IN_ACL_AFTER_LB_EVAL, 0, "1",
                      "next;", NULL);
    ...
    if (has_stateful) {
        ...
        ovn_lflow_add(lflows, od, S_SWITCH_IN_ACL_EVAL, 1,
                      ...
                      REGBIT_ACL_VERDICT_ALLOW" = 1; next;", NULL);
        ...
        ds_put_format(&match, "!ct.est && ct.rel && !ct.new%s && %s == 0",
                      ...
                      ct_blocked_match);
        ovn_lflow_add(lflows, od, S_SWITCH_IN_ACL_EVAL, UINT16_MAX - 3,
                      ds_cstr(&match), ct_in_acl_action, NULL);
        ovn_lflow_add(lflows, od, S_SWITCH_OUT_ACL_EVAL, UINT16_MAX - 3,
                      ds_cstr(&match), ct_out_acl_action, NULL);
        ...
        if (od->nbs->ports[i]->dhcpv4_options) {
            ...
            if (server_id && server_mac && lease_time) {
                ...
                ovn_lflow_add_with_lport_and_hint(
                    ...
                    &od->nbs->ports[i]->dhcpv4_options->header_, NULL);
            }
        }
        ...
        if (od->nbs->ports[i]->dhcpv6_options) {
            ...
            if (server_mac && eth_addr_from_string(server_mac, &ea)) {
                ...
                ovn_lflow_add_with_lport_and_hint(
                    ...
                    &od->nbs->ports[i]->dhcpv6_options->header_, NULL);
            }
        }
    }
...
build_qos(struct ovn_datapath *od, struct hmap *lflows) {
    ...
    ovn_lflow_add(lflows, od, S_SWITCH_IN_QOS_MARK, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_OUT_QOS_MARK, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_IN_QOS_METER, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_OUT_QOS_METER, 0, "1", "next;", NULL);
                           ...
                           const struct ovn_datapaths *lr_datapaths)
{
    ...
    for (size_t i = 0; i < lb_vip->n_backends; i++) {
        ...
        ovn_lflow_add_with_dp_group(
            ...
            ds_cstr(&aff_action_learn), &lb->nlb->header_, NULL);
...
{
    ovs_assert(od->nbs);
    ovn_lflow_add(lflows, od, S_SWITCH_IN_LB_AFF_CHECK, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_IN_LB_AFF_LEARN, 0, "1", "next;", NULL);
}
...
{
    ovs_assert(od->nbr);
    ovn_lflow_add(lflows, od, S_ROUTER_IN_LB_AFF_CHECK, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_ROUTER_IN_LB_AFF_LEARN, 0, "1", "next;", NULL);
}
    ...
    for (size_t i = 0; i < lb->n_vips; i++) {
        ...
        if (reject) {
            ...
            BITMAP_FOR_EACH_1 (index, ods_size(ls_datapaths),
                ...
                ovn_lflow_add_with_hint__(
                        ...
                        NULL, meter, &lb->nlb->header_, NULL);
            }
        }
        if (!reject || build_non_meter) {
            ovn_lflow_add_with_dp_group(
                ...
                ds_cstr(match), ds_cstr(action), &lb->nlb->header_, NULL);
        }
               ...
               struct hmap *lflows)
{
     ...
     * allowed by default. */
    ovn_lflow_add(lflows, od, S_SWITCH_IN_LB, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_IN_STATEFUL, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_OUT_STATEFUL, 0, "1", "next;", NULL);
...
build_lb_hairpin(struct ovn_datapath *od, struct hmap *lflows)
{
     ...
     */
    ovn_lflow_add(lflows, od, S_SWITCH_IN_PRE_HAIRPIN, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_IN_NAT_HAIRPIN, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_IN_HAIRPIN, 0, "1", "next;", NULL);
...
build_vtep_hairpin(struct ovn_datapath *od, struct hmap *lflows)
{
    ...
    ovn_lflow_add(lflows, od, S_SWITCH_IN_HAIRPIN, 1000,
                  REGBIT_FROM_RAMP" == 1", action, NULL);
    free(action);
...
build_lswitch_rport_arp_req_self_orig_flow(struct ovn_port *op,
                                           ...
                                           struct ovn_datapath *od,
                                           struct hmap *lflows,
                                           struct ovs_list *op_lflows_ref_list)
{
    ...
    ovn_lflow_add(lflows, od, S_SWITCH_IN_L2_LKUP, priority, ds_cstr(&match),
                  "outport = \""MC_FLOOD_L2"\"; output;", op_lflows_ref_list);
                                  ...
                                  struct ovs_list *op_lflows_ref_list)
{
    ...
    for (size_t i = 0; i < op->lrp_networks.n_ipv4_addrs; i++) {
        build_lswitch_rport_arp_req_flow(
            ...
            lflows, stage_hint, op_lflows_ref_list);
    }
    for (size_t i = 0; i < op->lrp_networks.n_ipv6_addrs; i++) {
        build_lswitch_rport_arp_req_flow(
    ...
    for (size_t j = 0; j < lsp_addrs->n_ipv4_addrs; j++) {
        ...
        if (build_dhcpv4_action(
            ...
            ovn_lflow_add_with_lport_and_hint(
                ...
                &op->nbsp->dhcpv4_options->header_, &op->lflows);
    ...
    for (size_t j = 0; j < lsp_addrs->n_ipv6_addrs; j++) {
        ...
        if (build_dhcpv6_action(
            ...
            ovn_lflow_add_with_lport_and_hint(
                lflows, op->od, S_SWITCH_IN_DHCP_RESPONSE, 100,
                ds_cstr(&match), ds_cstr(&response_action), inport->key,
                &op->nbsp->dhcpv6_options->header_, &op->lflows);
            ds_destroy(&options_action);
            ds_destroy(&response_action);
            break;
        }
                                ...
                                struct hmap *lflows)
{
    ...
    if (od->has_unknown) {
        ovn_lflow_add(lflows, od, S_SWITCH_IN_L2_UNKNOWN, 50,
                      "outport == \"none\"",
                      "outport = \""MC_UNKNOWN "\"; output;", NULL);
    } else {
        ovn_lflow_add(lflows, od, S_SWITCH_IN_L2_UNKNOWN, 50,
                      "outport == \"none\"",  debug_drop_action(), NULL);
    }
                                       ...
                                       struct hmap *lflows)
{
    ...
    ovn_lflow_add(lflows, od, S_SWITCH_IN_CHECK_PORT_SEC, 50, "1",
                  REGBIT_PORT_SEC_DROP" = check_in_port_sec(); next;", NULL);
...
{
    ovs_assert(od->nbs);
    ovn_lflow_add(lflows, od, S_SWITCH_IN_ARP_ND_RSP, 0, "1", "next;", NULL);
}
...
{
    ovs_assert(od->nbs);
    ovn_lflow_add(lflows, od, S_SWITCH_IN_DHCP_OPTIONS, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_IN_DHCP_RESPONSE, 0, "1", "next;",
                  NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_IN_DNS_LOOKUP, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_SWITCH_IN_DNS_RESPONSE, 0, "1", "next;", NULL);
                                        ...
                                        const struct shash *meter_groups)
{
    ...
    if (mcast_sw_info->enabled) {
        ...
        ovn_lflow_add(lflows, od, S_SWITCH_IN_L2_LKUP, 85,
                      ...
                      "outport = \""MC_FLOOD_L2"\"; output;", NULL);
                                ...
                                uint16_t ecmp_group_id)
{
    ...
    for (size_t i = 0; i < rule->n_nexthops; i++) {
        ...
        ovn_lflow_add_with_hint(lflows, od, S_ROUTER_IN_POLICY_ECMP,
                                100, ds_cstr(&match),
                                ds_cstr(&actions), &rule->header_, NULL);
    }
...

{
    ...
    ovn_lflow_add(lflows, od, S_ROUTER_IN_IP_ROUTING, priority,
                  ds_cstr(&route_match), ds_cstr(&actions), NULL);
...
add_route(struct hmap *lflows, struct ovn_datapath *od,
          ...
          int ofs, struct ovs_list *op_lflows_ref_list)
{
                                  ...
                                  const unsigned long *dp_bitmap)
{
    ...
    if (ctx->reject) {
        ...
        BITMAP_FOR_EACH_1 (index, bitmap_len, dp_bitmap) {
            ...
            ovn_lflow_add_with_hint__(ctx->lflows, od, S_ROUTER_IN_DNAT,
                    ...
                    NULL, meter, &ctx->lb->nlb->header_, NULL);
        }
    }
    if (!ctx->reject || build_non_meter) {
        ovn_lflow_add_with_dp_group(ctx->lflows,
            ...
            ctx->new_action[type], &ctx->lb->nlb->header_, NULL);
    }
    bitmap_free(dp_non_meter);
}
    ...
    if (lb->skip_snat) {
        BITMAP_FOR_EACH_1 (index, ods_size(lr_datapaths), lb_dps->nb_lr_map) {
            ...
            ovn_lflow_add(lflows, od, S_ROUTER_OUT_SNAT, 120,
                          "flags.skip_snat_for_lb == 1 && ip", "next;", NULL);
        }
    }
}
                       ...
                       struct hmap *lflows)
{
    ...
    ovn_lflow_add_with_hint(lflows, od, S_ROUTER_IN_IP_INPUT, priority,
                            ds_cstr(&match), ds_cstr(&actions), hint, NULL);
        ...
        struct ovn_datapath *od, struct hmap *lflows)
{
    ...
    ovn_lflow_add_default_drop(lflows, od, S_ROUTER_IN_ADMISSION, NULL);
}
        ...
        const struct shash *meter_groups)
{
    ...
    ovn_lflow_add(lflows, od, S_ROUTER_IN_LOOKUP_NEIGHBOR, 0, "1",
                  REGBIT_LOOKUP_NEIGHBOR_RESULT" = 1; next;", NULL);
    ...
    ovn_lflow_add_default_drop(lflows, od, S_ROUTER_IN_LEARN_NEIGHBOR, NULL);
}
                                       ...
                                       struct hmap *lflows)
{
    ...
    ovn_lflow_add(lflows, od, S_ROUTER_IN_IP_ROUTING_PRE, 0, "1",
                  REG_ROUTE_TABLE_ID" = 0; next;", NULL);
}
        ...
        const struct hmap *bfd_connections)
{
    ...
    ovn_lflow_add_default_drop(lflows, od, S_ROUTER_IN_IP_ROUTING_ECMP,
                               NULL);
    ovn_lflow_add_default_drop(lflows, od, S_ROUTER_IN_IP_ROUTING, NULL);
    ovn_lflow_add(lflows, od, S_ROUTER_IN_IP_ROUTING_ECMP, 150,
                  REG_ECMP_GROUP_ID" == 0", "next;", NULL);
        ...
        const struct hmap *lr_ports)
{
    ...
    ovn_lflow_add(lflows, od, S_ROUTER_IN_POLICY_ECMP, 150,
                  REG_ECMP_GROUP_ID" == 0", "next;", NULL);
    ovn_lflow_add_default_drop(lflows, od, S_ROUTER_IN_POLICY_ECMP, NULL);
        ...
        struct ovn_datapath *od, struct hmap *lflows)
{
    ...
    ovn_lflow_add(lflows, od, S_ROUTER_IN_ARP_RESOLVE, 500,
                  "ip4.mcast || ip6.mcast", "next;", NULL);

    ovn_lflow_add(lflows, od, S_ROUTER_IN_ARP_RESOLVE, 1, "ip4",
                  "get_arp(outport, " REG_NEXT_HOP_IPV4 "); next;", NULL);

    ovn_lflow_add(lflows, od, S_ROUTER_IN_ARP_RESOLVE, 1, "ip6",
                  "get_nd(outport, " REG_NEXT_HOP_IPV6 "); next;", NULL);

    ovn_lflow_add_default_drop(lflows, od, S_ROUTER_IN_ARP_RESOLVE, NULL);
}
                             ...
                             struct ovs_list *op_lflows_ref_list)
{
    ...
    for (size_t i = 0; i < ra->n_addrs; i++) {
        ...
        ovn_lflow_add(lflows, peer->od, S_ROUTER_IN_ARP_RESOLVE, 100,
                      ds_cstr(match), ds_cstr(actions), op_lflows_ref_list);
    }
}
        ...
        struct ds *match, struct ds *actions)
{
        ...
        for (int j = 0; j < od->n_nat_entries; j++) {
            ...
            if (nat->nb->allowed_ext_ips) {
                ovn_lflow_add_with_hint(lflows, od, S_ROUTER_IN_GW_REDIRECT,
                                        ...
                                        ds_cstr(actions), stage_hint, NULL);
                if (add_def_flow) {
                    ds_clear(&match_ext);
    ...
    /* Packets are allowed by default. */
    ovn_lflow_add(lflows, od, S_ROUTER_IN_GW_REDIRECT, 0, "1", "next;", NULL);
}
        ...
        struct ds *match, struct ds *actions)
{
    ...
    ovn_lflow_add(lflows, op->od, S_ROUTER_OUT_DELIVERY, 100,
                  ds_cstr(match), "output;", NULL);

    ovn_lflow_add_default_drop(lflows, op->od, S_ROUTER_OUT_DELIVERY, NULL);
}
        ...
        struct ovn_datapath *od, struct hmap *lflows)
{
    ...
    if (od->mcast_info.rtr.flood_static) {
        ovn_lflow_add(lflows, od, S_ROUTER_IN_IP_INPUT, 120,
                      "igmp && ip.ttl == 1", "next;", NULL);
        ovn_lflow_add(lflows, od, S_ROUTER_IN_IP_INPUT, 120,
                      "(mldv1 || mldv2) && ip.ttl == 1", "next;", NULL);
    }
     ...
     * routing. */
    ovn_lflow_add(lflows, od, S_ROUTER_IN_IP_INPUT, 0, "1", "next;", NULL);
}
        ...
        struct ds *match)
{
    ...
    for (size_t i = 0; i < op->lrp_networks.n_ipv6_addrs; i++) {
        ...
        ovn_lflow_add(lflows, op->od, S_ROUTER_IN_IP_INPUT, 100,
                      ds_cstr(match),
                      "reg0 = 0; handle_dhcpv6_reply;", NULL);
    }
}
                                      ...
                                      bool is_v6, struct ovn_port *l3dgw_port)
{
    ...
    ovn_lflow_add_with_hint(lflows, od, S_ROUTER_OUT_SNAT,
                            priority, ds_cstr(match),
                            ds_cstr(actions), &nat->header_, NULL);
}
                                     ...
                                     bool is_v6, struct ovn_port *l3dgw_port)
{
    ...
    ovn_lflow_add_with_hint(lflows, od, S_ROUTER_OUT_SNAT,
                            priority + 1, ds_cstr(match),
                            ds_cstr(&zone_actions), &nat->header_, NULL);
                                ...
                                const struct chassis_features *features)
{
    ...
    /* Packets are allowed by default. */
    ovn_lflow_add(lflows, od, S_ROUTER_IN_DEFRAG, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_ROUTER_IN_UNSNAT, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_ROUTER_OUT_CHECK_DNAT_LOCAL, 0, "1",
                  REGBIT_DST_NAT_IP_LOCAL" = 0; next;", NULL);
    ovn_lflow_add(lflows, od, S_ROUTER_OUT_SNAT, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_ROUTER_IN_DNAT, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_ROUTER_OUT_UNDNAT, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_ROUTER_OUT_POST_UNDNAT, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_ROUTER_OUT_POST_SNAT, 0, "1", "next;", NULL);
    ovn_lflow_add(lflows, od, S_ROUTER_OUT_EGR_LOOP, 0, "1", "next;", NULL);
    ...
    if (od->has_lb_vip && features->ct_lb_related) {
        ...
        ovn_lflow_add(lflows, od, S_ROUTER_IN_DNAT, 70, ds_cstr(match),
                      "flags.skip_snat_for_lb = 1; ct_commit_nat;", NULL);

        ds_truncate(match, match_len);
        ds_put_format(match, " && %s.force_snat == 1", ct_flag_reg);
        ovn_lflow_add(lflows, od, S_ROUTER_IN_DNAT, 70, ds_cstr(match),
                      "flags.force_snat_for_lb = 1; ct_commit_nat;", NULL);
    ...
    if (od->has_lb_vip) {
        ...
        ds_put_format(match, " && %s.skip_snat == 1", ct_flag_reg);
        ovn_lflow_add(lflows, od, S_ROUTER_IN_DNAT, 70, ds_cstr(match),
                      "flags.skip_snat_for_lb = 1; next;", NULL);

        ds_truncate(match, match_len);
        ds_put_format(match, " && %s.force_snat == 1", ct_flag_reg);
        ovn_lflow_add(lflows, od, S_ROUTER_IN_DNAT, 70, ds_cstr(match),
                      "flags.force_snat_for_lb = 1; next;", NULL);
    ...
    if (od->is_gw_router && (od->nbr->n_nat || od->has_lb_vip)) {
        ...
        ovn_lflow_add(lflows, od, S_ROUTER_OUT_UNDNAT, 100,
                      "nd || nd_rs || nd_ra", "next;", NULL);
        ovn_lflow_add(lflows, od, S_ROUTER_OUT_UNDNAT, 50,
                      "ip", "flags.loopback = 1; ct_dnat;", NULL);
        ovn_lflow_add(lflows, od, S_ROUTER_OUT_POST_UNDNAT, 50,
                      "ip && ct.new", "ct_commit { } ; next;", NULL);
    }

    /* Send the IPv6 NS packets to next table. When ovn-controller
     * generates IPv6 NS (for the action - nd_ns{}), the injected
     * packet would go through conntrack - which is not required. */
    ovn_lflow_add(lflows, od, S_ROUTER_OUT_SNAT, 120, "nd_ns", "next;", NULL);
