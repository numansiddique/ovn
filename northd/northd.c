...
static void
ovn_lb_svc_create(struct ovsdb_idl_txn *ovnsb_txn, const struct ovn_northd_lb *lb,
...
associate_ls_lbs(struct ovn_datapath *ls_od, struct hmap *lb_datapaths_map,
                 bool update_lb_dps_count, size_t n_ls_datapaths)
{
                   ...
                   struct hmap *lb_group_datapaths_map)
{
    ...
    HMAP_FOR_EACH (od, key_node, &ls_datapaths->datapaths) {
        ...
        associate_ls_lbs(od, lb_datapaths_map, false, ods_size(ls_datapaths));
    ...
    HMAP_FOR_EACH (od, key_node, &lr_datapaths->datapaths) {
        ...
        for (size_t i = 0; i < od->nbr->n_load_balancer_group; i++) {
            ...
            if (!od->lb_ips) {
                od->lb_ips = ovn_lb_ip_set_clone(lb_group_dps->lb_group->lb_ips);
            } else {
                for (size_t j = 0; j < lb_group_dps->lb_group->n_lbs; j++) {
                    build_lrouter_lb_ips(od->lb_ips, lb_group_dps->lb_group->lbs[j]);
                }
            }
        }
                                ...
                                struct hmap *lb_group_dps_map)
{
    ...
    HMAP_FOR_EACH (od, key_node, &lr_datapaths->datapaths) {
        ...
        for (size_t i = 0; i < od->nbr->n_load_balancer; i++) {
            struct ovn_lb_datapaths *lb_dps =
                ovn_lb_datapaths_find(lb_dps_map, &od->nbr->load_balancer[i]->header_.uuid);
            ovs_assert(lb_dps);
         ...
         struct ovn_datapaths *ls_datapaths, struct hmap *lb_dps_map)
{
    ...
    SBREC_LOAD_BALANCER_TABLE_FOR_EACH_SAFE (sbrec_lb,
        ...
        lb_dps->dpg = ovn_dp_group_get_or_create(ovnsb_txn, &dp_groups,
                                             lb_dps->slb->datapath_group,
    ...
    HMAP_FOR_EACH (lb_dps, hmap_node, lb_dps_map) {
        ...
        /* Update columns. */
        sbrec_load_balancer_set_name(lb_dps->slb, lb_dps->lb->nlb->name);
        sbrec_load_balancer_set_vips(lb_dps->slb,
                                     ovn_northd_lb_get_vips(lb_dps->lb));
        sbrec_load_balancer_set_protocol(lb_dps->slb, lb_dps->lb->nlb->protocol);
        sbrec_load_balancer_set_datapath_group(lb_dps->slb,
                                               lb_dps->dpg->dp_group);
        sbrec_load_balancer_set_options(lb_dps->slb, &options);
        /* Clearing 'datapaths' column, since 'dp_group' is in use. */
        sbrec_load_balancer_set_datapaths(lb_dps->slb, NULL, 0);
        smap_destroy(&options);
    }
                               ...
                               bool *updated)
{
    ...
    associate_ls_lbs(od, lb_datapaths_map, true, n_ls_datapaths);
    ls_change->lbs_changed = true;
    *updated = true;
    return true;
}
                              ...
                              struct hmap *lb_group_datapaths_map)
{
    ...
    LIST_FOR_EACH (trk_lb, list_node,
        ...
        lb_dps = ovn_lb_datapaths_find(lb_datapaths_map, lb_uuid);
        if (!lb_dps) {
            lb_dps = ovn_lb_datapaths_create(trk_lb->lb, ods_size(ls_datapaths),
                                             ods_size(lr_datapaths));
            hmap_insert(lb_datapaths_map, &lb_dps->hmap_node,
                        uuid_hash(lb_uuid));
        }
...
static void
build_lb_rules_pre_stateful(struct hmap *lflows, struct ovn_lb_datapaths *lb_dps,
                            bool ct_lb_mark,
                            const struct ovn_datapaths *ls_datapaths,
                            struct ds *match, struct ds *action)
{
 ...
 */
static void
build_lb_affinity_ls_flows(struct hmap *lflows, struct ovn_lb_datapaths *lb_dps,
                           ...
                           const struct ovn_datapaths *ls_datapaths)
{
    ...
    for (size_t i = 0; i < lb_vip->n_backends; i++) {
        ...
        /* Use already selected backend within affinity timeslot. */
        ovn_lflow_add_with_dp_group(
            lflows, lb_dps->nb_ls_map, ods_size(ls_datapaths), S_SWITCH_IN_LB, 150,
            ds_cstr(&aff_match), ds_cstr(&aff_action), &lb->nlb->header_);
...
static void
build_lswitch_flows_for_lb(struct ovn_lb_datapaths *lb_dps, struct hmap *lflows,
                           const struct shash *meter_groups,
                           const struct ovn_datapaths *ls_datapaths,
                           const struct chassis_features *features,
                           const struct hmap *svc_monitor_map,
    ...
    if (parallelization_state == STATE_USE_PARALLELIZATION) {
        ...
        HMAP_FOR_EACH (lb_dps, hmap_node, lb_dps_map) {
            build_lswitch_arp_nd_service_monitor(lb_dps->lb, lsi.ls_ports,
                                                 ...
                                                 &lsi.match);
            build_lrouter_defrag_flows_for_lb(lb_dps, lsi.lflows, lsi.lr_datapaths,
                                              &lsi.match);
...
northd_destroy(struct northd_data *data)
{
    ...
    struct ovn_lb_group_datapaths *lb_group_dps;
    HMAP_FOR_EACH_POP (lb_group_dps, hmap_node, &data->lb_group_datapaths_map) {
        ovn_lb_group_datapaths_destroy(lb_group_dps);
    }
