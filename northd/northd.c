                               ...
                               struct lflow_ref *lflow_ref)
{
    ...
    BITMAP_FOR_EACH_1 (index, bitmap_len, lb_dps->nb_lr_map) {
        ...
        const struct lr_lb_nat_data_record *lr_lbnat_rec =
            lr_lb_nat_data_table_find(lr_lbnats, od->nbr);
        ovs_assert(lr_lbnat_rec);
    ...
    for (size_t i = 0; i < peer_ls->n_router_ports; i++) {
        ...
        if (!dynamic_neigh_router &&
            ...
            for (size_t k = 0; k < ra.n_addrs; k++) {
                ...
                ds_clear(actions);
                ds_put_format(actions, "eth.dst = %s; next;", ra.laddrs[k].ea_s);
                ovn_lflow_add(lflows, router_port->od, S_ROUTER_IN_ARP_RESOLVE, 100,
                            ds_cstr(match), ds_cstr(actions), lflow_ref);
            }
        }
    }
                                  ...
                                  struct lflow_ref *lflow_ref)
{
    ...
    const struct lr_lb_nat_data_record *lr_lbnat_rec;
    lr_lbnat_rec = lr_lb_nat_data_table_find(lr_lbnats, op->peer->od->nbr);
    ovs_assert(lr_lbnat_rec);
                                ...
                                struct lflow_table *lflows)
{
    ...
    HMAPX_FOR_EACH (hmapx_node, &trk_data->crupdated) {
        ...
        /* Sync the new flows to SB. */
        lflow_ref_sync_lflows_to_sb(ls_lbacls_rec->lflow_ref, lflows, ovnsb_txn,
                             lflow_input->ls_datapaths,
                             lflow_input->lr_datapaths,
                             lflow_input->ovn_internal_version_changed,
                             lflow_input->sbrec_logical_flow_table,
