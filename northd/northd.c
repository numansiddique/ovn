...
/* Sync the SB Port bindings for the tracked logical switch ports in the
 * tracked logical switches (from the northd engine node). */
bool
                              ...
                              struct hmap *lbgrp_datapaths_map)
{
    ...
    HMAP_FOR_EACH (crupdated_lbgrp, hmap_node, &trk_lb_data->crupdated_lbgrps) {
        lbgrp = crupdated_lbgrp->lbgrp;
        const struct uuid *lb_uuid = &lbgrp->uuid;
