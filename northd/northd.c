                ...
                struct ovs_list *lr_list)
{
    ...
    LIST_FOR_EACH_SAFE (od, next, list, &both) {
        ovn_datapath_allocate_key(data, datapaths, &dp_tnlids, od, &hint);
    }
...
static void
build_ovn_lb_svcs(struct northd_data *data, struct ovsdb_idl_txn *ovnsb_txn,
...
static void
build_bfd_table(struct northd_data *data, struct ovsdb_idl_txn *ovnsb_txn,
...
void build_lflows(struct northd_data *data, struct ovsdb_idl_txn *ovnsb_txn)
{
    struct hmap lflows;
...
static void
sync_address_sets(struct northd_data *data, struct ovsdb_idl_txn *ovnsb_txn,
                  struct hmap *datapaths)
{
 ...
 */
static void
sync_port_groups(struct northd_data *data, struct ovsdb_idl_txn *ovnsb_txn,
                 struct hmap *pgs)
{
...
static void
sync_dns_entries(struct northd_data *data, struct ovsdb_idl_txn *ovnsb_txn,
                 struct hmap *datapaths)
{
             ...
             int64_t loop_start_time)
{
    ...
    handle_port_binding_changes(data, ovnsb_txn, ports, &ha_ref_chassis_map);
