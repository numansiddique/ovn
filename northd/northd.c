...
get_op_routable_addresses(struct ovn_port *op,
                          const struct lr_lb_data_record *lrlb_rec)
{
    size_t n;
    char **nats = get_nat_addresses(op, &n, true, true, lrlb_rec);
                  ...
                  const struct lr_lb_data_record *lrlb_rec)
{
    ...
    if (include_lb_ips && lrlb_rec) {
        const char *ip_address;
        if (routable_only) {
            SSET_FOR_EACH (ip_address, &lrlb_rec->lb_ips->ips_v4_routable) {
                ds_put_format(&c_addresses, " %s", ip_address);
                central_ip_address = true;
            }
            SSET_FOR_EACH (ip_address, &lrlb_rec->lb_ips->ips_v6_routable) {
                ds_put_format(&c_addresses, " %s", ip_address);
                central_ip_address = true;
            }
        } else {
            SSET_FOR_EACH (ip_address, &lrlb_rec->lb_ips->ips_v4) {
                ds_put_format(&c_addresses, " %s", ip_address);
                central_ip_address = true;
            }
            SSET_FOR_EACH (ip_address, &lrlb_rec->lb_ips->ips_v6) {
                ds_put_format(&c_addresses, " %s", ip_address);
                central_ip_address = true;
            }
        }
...
static void
sync_pb_for_lsp(struct ovn_port *op, const struct lr_lb_data_table *lr_lbs)
{
    ovs_assert(op->nbsp);
    ...
    if (lsp_is_router(op->nbsp)) {
        ...
        if (nat_addresses && !strcmp(nat_addresses, "router")) {
            if (op->peer && op->peer->od
                ...
                const struct lr_lb_data_record *lrlb_rec = NULL;

                if (include_lb_vips) {
                    lrlb_rec = lr_lb_data_table_find(lr_lbs,
                                                     op->peer->od->nbr);
                }
                nats = get_nat_addresses(op->peer, &n_nats, false,
                                         include_lb_vips, lrlb_rec);
            }
...
sync_pbs(struct ovsdb_idl_txn *ovnsb_idl_txn, struct hmap *ls_ports,
         ...
         const struct lr_lb_data_table *lr_lbs)
{
    ovs_assert(ovnsb_idl_txn);

    struct ovn_port *op;
    HMAP_FOR_EACH (op, key_node, ls_ports) {
        sync_pb_for_lsp(op, lr_lbs);
    }
                      ...
                      bool *updated)
{
    ...
    /* If there were only router ports before the change or if there are only
                                  ...
                                  const struct ovsdb_idl_row *stage_hint)
{
    ...
    if (op->od->nbr->n_load_balancer || op->od->nbr->n_load_balancer_group) {
        lrlb_rec = lr_lb_data_table_find(lr_lbs, op->od->nbr);
        ovs_assert(lrlb_rec);

        const char *ip_addr;
        SSET_FOR_EACH (ip_addr, &lrlb_rec->lb_ips->ips_v4_reachable) {
            ovs_be32 ipv4_addr;
        ...
        SSET_FOR_EACH (ip_addr, &lrlb_rec->lb_ips->ips_v6_reachable) {
    ...
    for (size_t i = 0; i < lrnat_rec->n_nat_entries; i++) {
        ...
        if (nat_entry_is_v6(nat_entry)) {
            if (!lrlb_rec || !sset_contains(&lrlb_rec->lb_ips->ips_v6,
                                            nat->external_ip)) {
                build_lswitch_rport_arp_req_flow(
            ...
            if (!lrlb_rec || !sset_contains(&lrlb_rec->lb_ips->ips_v4,
                                            nat->external_ip)) {
                build_lswitch_rport_arp_req_flow(
    ...
    if (op->lrp_networks.n_ipv4_addrs) {
        for (size_t i = 0; i < op->lrp_networks.n_ipv4_addrs; i++) {
            ...
            bool router_ip_in_snat_ips = !!shash_find(&lrnat_rec->snat_ips,
                                                      ip);
            bool router_ip_in_lb_ips = (lrlb_rec &&
                                        !!sset_find(&lrlb_rec->lb_ips->ips_v4,
                                                    ip));
    ...
    if (op->lrp_networks.n_ipv6_addrs) {
        ...
        for (size_t i = 0; i < op->lrp_networks.n_ipv6_addrs; i++) {
            ...
            bool router_ip_in_snat_ips = !!shash_find(&lrnat_rec->snat_ips,
                                                      ip);
            bool router_ip_in_lb_ips = (lrlb_rec &&
                                        !!sset_find(&lrlb_rec->lb_ips->ips_v6,
                                                    ip));
            bool drop_router_ip = (drop_snat_ip == (router_ip_in_snat_ips ||
                                                    router_ip_in_lb_ips));
...
build_ip_routing_flows_for_router_type_lsp(
        struct ovn_port *op, const struct lr_lb_data_table *lr_lbs,
        ...
        struct hmap *lflows, struct ds *match, struct ds *actions)
{
     ...
     * Priority 2.
     */
    build_lrouter_drop_own_dest(op, lrnat_rec, lrlb_rec,
    ...
    if (op->od->n_router_ports && !lsp_is_router(op->nbsp)
        ...
        for (size_t i = 0; i < op->od->n_router_ports; i++) {
            ...
            if (smap_get(&peer->od->nbr->options, "chassis")
                || peer->cr_port) {
                const struct lr_lb_data_record *lrlb_rec;
                lrlb_rec = lr_lb_data_table_find(lr_lbs, router_port->od->nbr);
                routable_addresses_to_lflows(lflows, router_port, peer,
                                             lrlb_rec, match, actions);
            }
        }
    }
}
                            ...
                            const struct shash *meter_groups)
{
    ...
    if (lrlb_rec && sset_count(&lrlb_rec->lb_ips->ips_v4_reachable)) {
        ds_clear(match);
        if (is_l3dgw_port(op)) {
            ds_put_format(match, "is_chassis_resident(%s)",
                          op->cr_port->json_key);
    ...
    if (lrlb_rec && sset_count(&lrlb_rec->lb_ips->ips_v6_reachable)) {
        ds_clear(match);
                                         ...
                                         struct hmap *lflows)
{
    ...
    build_lswitch_ip_unicast_lookup(op, lr_nats, lr_lbs, lflows, actions,
                                    match);

    /* Build Logical Router Flows. */
    build_ip_routing_flows_for_router_type_lsp(op, lr_lbs, lr_ports, lflows);
    build_arp_resolve_flows_for_lsp(op, lflows, lr_ports, lr_lbs,
                                    match, actions);
                                         ...
                                         struct lswitch_flow_build_info *lsi)
{
    ...
    const struct lr_lb_data_record *lrlb_rec =
        lr_lb_data_table_find(lsi->lr_lbs, op->od->nbr);
    ...
    build_ND_RA_flows_for_lrouter_port(op, lsi->lflows, &lsi->match,
                                       &lsi->actions, lsi->meter_groups);
    build_arp_resolve_flows_for_lrp(op, lrnet_rec, lrlb_rec, lsi->lflows,
                                    &lsi->match, &lsi->actions);
    ...
    build_ipv6_input_flows_for_lrouter_port(op, lsi->lflows,
                                            &lsi->match, &lsi->actions,
                                            lsi->meter_groups);
    build_lrouter_ipv4_ip_input(op, lsi->lflows, lrnet_rec, lrlb_rec,
