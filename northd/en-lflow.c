                     ...
                     void *data)
{
    ...
    if (!lflow_handle_northd_port_changes(eng_ctx->ovnsb_idl_txn,
                                &northd_data->trk_northd_changes.trk_lsps,
...
lflow_lr_stateful_handler(struct engine_node *node, void *data)
{
    ...
    if (!lr_sful_data->tracked
        || lr_sful_data->tracked_data.vip_nats_changed
        || !hmapx_is_empty(&lr_sful_data->tracked_data.deleted)) {
        return false;
    }
...
lflow_ls_stateful_handler(struct engine_node *node, void *data)
{
    ...
    if (!ls_sful_data->tracked ||
            !hmapx_is_empty(&ls_sful_data->tracked_data.deleted)) {
        return false;
    }
