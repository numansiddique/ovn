...
static void lb_data_init(struct lb_data *);
static void lb_data_destroy(struct lb_data *);
...
static void destroy_tracked_data(struct lb_data *);
...
static inline void add_deleted_lb_to_tracked_data(struct ovn_northd_lb *,
                                                  struct tracked_lb_data *,
                                                  bool health_checks);
static inline void add_crupdated_lb_group_to_tracked_data(
...
{
    struct lb_data *data = xzalloc(sizeof *data);
    lb_data_init(data);
    return data;
}
...
{
    struct lb_data *lb_data = (struct lb_data *) data;
    ...
    NBREC_LOAD_BALANCER_GROUP_TABLE_FOR_EACH_TRACKED (tracked_lb_group,
        ...
        if (nbrec_load_balancer_group_is_new(tracked_lb_group)) {
            ...
            HMAPX_FOR_EACH_SAFE (hmapx_node, &pre_update_lbs) {
                 ...
                 * just removed from the lb group. If it's just removed,
                 * then add it to the tracked lbs */
...
/* static functions. */
static void
lb_data_init(struct lb_data *lb_data)
{
...
static void
lb_data_destroy(struct lb_data *lb_data)
{
    struct ovn_northd_lb *lb;
    ...
    hmapx_destroy(&lb_data->tracked_lb_data.crupdated_lb_groups);
...
static void
destroy_tracked_data(struct lb_data *lb_data)
{
    lb_data->tracked = false;
    ...
    hmapx_clear(&lb_data->tracked_lb_data.crupdated_lb_groups);
}
...
add_crupdated_lb_group_to_tracked_data(struct ovn_lb_group *lbg,
                                       struct tracked_lb_data *tracked_lb_data)
{
    hmapx_add(&tracked_lb_data->crupdated_lb_groups, lbg);
}
