...
static void ls_stateful_record_set_acl_flags(struct ls_stateful_record *,
                                           const struct ovn_datapath *,
                                           const struct ls_port_group *,
                                           const struct ls_port_group_table *);
...
ls_stateful_table_find(
    const struct ls_stateful_table *table,
                  ...
                  struct engine_arg *arg OVS_UNUSED)
{
    ...
    ls_stateful_table_init(&data->ls_sful_table);
...
{
    struct ed_type_ls_stateful *data =
        (struct ed_type_ls_stateful *) data_;
    ls_stateful_table_destroy(&data->ls_sful_table);
...
en_ls_stateful_clear_tracked_data(void *data_)
{
    ...
    HMAPX_FOR_EACH_SAFE (hmapx_node, &data->tracked_data.deleted) {
        ls_stateful_record_destroy(hmapx_node->data);
...
en_ls_stateful_run(struct engine_node *node, void *data_)
{
    ...
    ls_stateful_table_clear(&data->ls_sful_table);
    ls_stateful_table_build(&data->ls_sful_table, input_data.ls_datapaths,
...
ls_stateful_northd_handler(struct engine_node *node, void *data_)
{
    ...
    HMAPX_FOR_EACH (hmapx_node, &nd_changes->ls_with_changed_lbs) {
        ...
        ls_stateful_rec = ls_stateful_table_find_(&data->ls_sful_table,
                                                  od->nbs);
        if (!ls_stateful_rec) {
            ls_stateful_rec = ls_stateful_record_create(
                &data->ls_sful_table, od, input_data.ls_port_groups);
        } else {
        ...
        hmapx_add(&data->tracked_data.crupdated, ls_stateful_rec);
    }
...
ls_stateful_port_group_handler(struct engine_node *node, void *data_)
{
    ...
    LS_PORT_GROUP_TABLE_FOR_EACH (ls_pg, input_data.ls_port_groups) {
        struct ls_stateful_record *ls_sful_rec =
            ls_stateful_table_find_(&data->ls_sful_table, ls_pg->nbs);
        ...
        if (!ls_sful_rec) {
            ...
            ovs_assert(od);
            ls_sful_rec = ls_stateful_record_create(
                &data->ls_sful_table, od, input_data.ls_port_groups);
            modified = true;
        } else {
            bool had_stateful_acl = ls_sful_rec->has_stateful_acl;
            uint64_t max_acl_tier = ls_sful_rec->max_acl_tier;
            bool had_acls = ls_sful_rec->has_acls;

            ls_stateful_record_reinit(ls_sful_rec, ls_pg,
                                      input_data.ls_port_groups);

            if ((had_stateful_acl != ls_sful_rec->has_stateful_acl)
                || (had_acls != ls_sful_rec->has_acls)
                || max_acl_tier != ls_sful_rec->max_acl_tier) {
                modified = true;
            }
        }

        if (modified) {
            /* Add the ls_sful_rec to the tracking data. */
            hmapx_add(&data->tracked_data.crupdated, ls_sful_rec);
        }
    }
...
bool
ls_stateful_logical_switch_handler(struct engine_node *node, void *data_)
{
    struct ls_stateful_input input_data = ls_stateful_get_input_data(node);
    const struct nbrec_logical_switch *nbs;
    struct ed_type_ls_stateful *data = data_;
    ...
    NBREC_LOGICAL_SWITCH_TABLE_FOR_EACH_TRACKED (nbs,
        ...
        struct ls_stateful_record *ls_sful_rec =
            ls_stateful_table_find_(&data->ls_sful_table, nbs);

        if (nbrec_logical_switch_is_deleted(nbs)) {
            if (ls_sful_rec) {
                /* Remove the record from the entries. */
                hmap_remove(&data->ls_sful_table.entries,
                            &ls_sful_rec->key_node);

                /* Add the ls_stateful_rec to the tracking data. */
                hmapx_add(&data->tracked_data.deleted, ls_sful_rec);
            }
        } else {
            if (!ls_sful_rec) {
                const struct ovn_datapath *od;
                ...
                ls_stateful_record_reinit(ls_sful_rec, NULL,
                                          input_data.ls_port_groups);
            }
...
static void
ls_stateful_record_init(struct ls_stateful_record *ls_sful_rec,
                      const struct ovn_datapath *od,
                      const struct ls_port_group *ls_pg,
                      const struct ls_port_group_table *ls_pgs)
{
    ls_sful_rec->has_lb_vip = ls_has_lb_vip(od);
    ls_stateful_record_set_acl_flags(ls_sful_rec, od, ls_pg, ls_pgs);
}

static void
ls_stateful_record_reinit(struct ls_stateful_record *ls_sful_rec,
                        const struct ls_port_group *ls_pg,
                        const struct ls_port_group_table *ls_pgs)
{
    ls_stateful_record_init(ls_sful_rec, ls_sful_rec->od, ls_pg, ls_pgs);
}
...
static void
ls_stateful_record_set_acl_flags(struct ls_stateful_record *ls_sful_rec,
                               const struct ovn_datapath *od,
                               const struct ls_port_group *ls_pg,
                               const struct ls_port_group_table *ls_pgs)
{
    ls_sful_rec->has_stateful_acl = false;
    ls_sful_rec->max_acl_tier = 0;
    ls_sful_rec->has_acls = false;

    if (ls_stateful_record_set_acl_flags_(ls_sful_rec, od->nbs->acls,
                                        od->nbs->n_acls)) {
        return;
    }
    ...
    HMAP_FOR_EACH (ls_pg_rec, key_node, &ls_pg->nb_pgs) {
        if (ls_stateful_record_set_acl_flags_(ls_sful_rec,
...
static bool
ls_stateful_record_set_acl_flags_(struct ls_stateful_record *ls_sful_rec,
                                ...
                                size_t n_acls)
{
    ...
    ls_sful_rec->has_acls = true;
    for (size_t i = 0; i < n_acls; i++) {
        const struct nbrec_acl *acl = acls[i];
        if (acl->tier > ls_sful_rec->max_acl_tier) {
            ls_sful_rec->max_acl_tier = acl->tier;
        }
        if (!ls_sful_rec->has_stateful_acl
                && !strcmp(acl->action, "allow-related")) {
            ls_sful_rec->has_stateful_acl = true;
        }
        if (ls_sful_rec->has_stateful_acl &&
            ls_sful_rec->max_acl_tier ==
