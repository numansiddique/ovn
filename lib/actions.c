...
format_DHCPV4_RELAY_REQ_CHK(const struct ovnact_dhcp_relay *dhcp_relay,
                            struct ds *s)
{
    format_dhcp_relay_chk("dhcp_relay_req_chk", dhcp_relay, s);
}
...
static void
encode_dhcp_relay_chk(const struct ovnact_dhcp_relay *dhcp_relay,
                      const struct ovnact_encode_params *ep,
                      struct ofpbuf *ofpacts, enum action_opcode relay_opcode)
{
    struct mf_subfield dst = expr_resolve_field(&dhcp_relay->dst);
    size_t oc_offset = encode_start_controller_op(relay_opcode, true,
                                                  ep->ctrl_meter_id, ofpacts);
...
encode_DHCPV4_RELAY_REQ_CHK(const struct ovnact_dhcp_relay *dhcp_relay,
                            ...
                            struct ofpbuf *ofpacts)
{
    encode_dhcp_relay_chk(dhcp_relay, ep, ofpacts,
...
format_DHCPV4_RELAY_RESP_CHK(const struct ovnact_dhcp_relay *dhcp_relay,
                             struct ds *s)
{
    format_dhcp_relay_chk("dhcp_relay_resp_chk", dhcp_relay, s);
}
