...
build_lswitch_stateful_acls(struct hmap *lflows, bool use_ct_inv_match)
{
    ...
    ovn_ctrl_lflow_add(lflows, S_SWITCH_IN_ACL, UINT16_MAX - 3, match,
                       "drop;");
    ovn_ctrl_lflow_add(lflows, S_SWITCH_OUT_ACL, UINT16_MAX - 3, match,
                       "drop;");
    free(match);
    ...
    match = xasprintf("ct.est && !ct.rel && !ct.new%s && "
                      ...
                      use_ct_inv_match ? " && !ct.inv" : "");
    ovn_ctrl_lflow_add(lflows, S_SWITCH_IN_ACL, UINT16_MAX - 3,
                       match, "next;");
    ovn_ctrl_lflow_add(lflows, S_SWITCH_OUT_ACL, UINT16_MAX - 3,
                       match, "next;");
    free(match);
                   ...
                   bool use_ct_inv_match)
{
    ...
    if (has_stateful) {
        build_lswitch_stateful_acl_hints(lflows);
        build_lswitch_stateful_acls(lflows, use_ct_inv_match);
    } else {
        ovn_ctrl_lflow_add(lflows, S_SWITCH_IN_ACL_HINT, UINT16_MAX,
                            "1", "next;");
        ovn_ctrl_lflow_add(lflows, S_SWITCH_OUT_ACL_HINT, UINT16_MAX,
                            "1", "next;");
                        ...
                        bool use_ct_inv_match)
{
    ...
    build_lswitch_acls(lflows, dp, use_ct_inv_match);
