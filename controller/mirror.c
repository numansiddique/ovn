...
static void sync_ovn_mirror(struct ovn_mirror *, struct port_mirror_ctx *);

static void create_ovs_mirror(struct ovn_mirror *, struct port_mirror_ctx *);
static void sync_ovs_mirror_ports(struct ovn_mirror *,
                                  struct port_mirror_ctx *);
static void delete_ovs_mirror(struct ovn_mirror *, struct port_mirror_ctx *);
...
void
ovn_port_mirror_init(struct shash *ovn_mirrors)
{
    shash_init(ovn_mirrors);
}

void
ovn_port_mirror_destroy(struct shash *ovn_mirrors)
{
...
void
ovn_port_mirror_run(struct port_mirror_ctx *pm_ctx, struct shash *ovn_mirrors)
{
    /* Iterate through sb mirrors and build the 'ovn_mirrors'. */
    const struct sbrec_mirror *sb_mirror;
    SBREC_MIRROR_TABLE_FOR_EACH (sb_mirror, pm_ctx->sb_mirror_table) {
    ...
    const struct ovsrec_mirror *ovs_mirror;
    OVSREC_MIRROR_TABLE_FOR_EACH (ovs_mirror, pm_ctx->mirror_table) {
    ...
    struct shash_node *node;
    SHASH_FOR_EACH (node, pm_ctx->local_bindings) {
        struct local_binding *lbinding = node->data;
        const struct sbrec_port_binding *pb =
            local_binding_get_primary_pb(pm_ctx->local_bindings, lbinding->name);
...
static void
sync_ovn_mirror(struct ovn_mirror *m, struct port_mirror_ctx *pm_ctx)
{
    if (should_delete_ovs_mirror(m)) {
        /* Delete the ovsrec mirror. */
        delete_ovs_mirror(m, pm_ctx);
        return;
    }
    ...
    if (m->sb_mirror && !m->ovs_mirror) {
        create_ovs_mirror(m, pm_ctx);
    }

    sync_ovs_mirror_ports(m, pm_ctx);
}
...
static void
create_ovs_mirror(struct ovn_mirror *m, struct port_mirror_ctx *pm_ctx)
{
    ...
    char *key = xasprintf("%ld", (long int) m->sb_mirror->index);
    smap_add(&options, "remote_ip", m->sb_mirror->sink);
    smap_add(&options, "key", key);
    if (!strcmp(m->sb_mirror->type, "erspan")) {
    ...
    struct ovsrec_port *port = ovsrec_port_insert(pm_ctx->ovs_idl_txn);
    ...
    m->ovs_mirror = ovsrec_mirror_insert(pm_ctx->ovs_idl_txn);
    ...
    ovsrec_bridge_update_mirrors_addvalue(pm_ctx->br_int, m->ovs_mirror);
}

static void
sync_ovs_mirror_ports(struct ovn_mirror *m, struct port_mirror_ctx *pm_ctx)
{
    struct mirror_lport *m_lport;
    ...
    if (ovs_list_is_empty(&m->mirror_src_lports)) {
        ...
        LIST_FOR_EACH (m_lport, list_node, &m->mirror_src_lports) {
            const struct ovsrec_port *p = get_iface_port(m_lport->lbinding->iface,
                                                         pm_ctx->br_int);
            ovs_assert(p);
...
static void
delete_ovs_mirror(struct ovn_mirror *m, struct port_mirror_ctx *pm_ctx)
{
    ovsrec_bridge_update_ports_delvalue(pm_ctx->br_int,
                                        m->ovs_mirror->output_port);
    ovsrec_bridge_update_mirrors_delvalue(pm_ctx->br_int, m->ovs_mirror);
