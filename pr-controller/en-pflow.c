...
static void pflow_run(const struct shash *pr_bridges);
static void physical_flows_add(const struct pr_bridge *);
...
/* Static functions. */
static void
pflow_run(const struct shash *pr_bridges)
{
    br_flow_switch_physical_oflow_tables();
    struct shash_node *shash_node;
    SHASH_FOR_EACH (shash_node, pr_bridges) {
        physical_flows_add(shash_node->data);
    }
}

static void
physical_flows_add(const struct pr_bridge *br)
{
    if (!br->ovs_br) {
        return;
    }
    ...
    /* Table 64, priority 0, actions=clear_IN_PORT and resubmit to 65
     * =============================================================
     *
     */
