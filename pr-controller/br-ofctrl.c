...
void
br_ofctrls_add_or_update_bridge(struct pr_bridge *br)
{
    ...
    if (!br_ofctrl) {
        ...
        br_ofctrl->swconn = rconn_create(0, 0, DSCP_DEFAULT, 1 << OFP15_VERSION);
...
/* ofctrl state machine functions. */

static void
ofctrl_recv(struct br_ofctrl *br_ofctrl, const struct ofp_header *oh,
            enum ofptype type)
{
...
run_S_WAIT_BEFORE_CLEAR(struct br_ofctrl *br_ofctrl)
{
    ...
    if (!br_ofctrl->wait_before_clear_expire) {
        /* Start the timer. */
        br_ofctrl->wait_before_clear_expire = time_msec() + br_ofctrl->wait_before_clear_time;
    }
...
run_S_CLEAR_FLOWS(struct br_ofctrl *br_ofctrl)
{
    ...
    br_ofctrl->ofctrl_initial_clear = true;
              ...
              bool lflows_changed, bool pflows_changed)
{
    ...
    if (lflows_changed || pflows_changed || br_ofctrl->skipped_last_time ||
        br_ofctrl->ofctrl_initial_clear) {
        need_put = true;
    ...
    if (!need_put) {
        VLOG_DBG("ofctrl_put not needed for bridge %s", br_ofctrl->bridge);
        return;
    }
    ...
    if (!br_ofctrl_can_put(br_ofctrl)) {
        VLOG_DBG("ofctrl_put can't be performed for bridge %s", br_ofctrl->bridge);
    ...
    if (br_ofctrl->ofctrl_initial_clear) {
        ...
        br_ofctrl->ofctrl_initial_clear = false;
    }
